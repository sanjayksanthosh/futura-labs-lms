...
import { User, Lock, Palette, Bell, Moon, Sun, Smartphone, Globe } from 'lucide-react';
...
export const MentorSettings = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl space-y-6">
      <div><h1 className="text-2xl font-bold">Settings</h1><p className="text-slate-500">Manage your account</p></div>
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <form onSubmit={handleProfile} className="space-y-4">
          ...
          <div><label className="label">Email</label><input value={user?.email || ''} disabled className="input-field opacity-60" /></div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="label">Role</label><input value="Mentor" disabled className="input-field opacity-60" /></div>
          </div>
          <div><label className="label">Bio</label><textarea value={profile.bio} onChange={e => setProfile(p => ({ ...p, bio: e.target.value }))} className="input-field" rows={2} placeholder="Tell us about yourself" /></div>
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        <h3 className="font-semibold flex items-center gap-2"><Palette className="h-4 w-4 text-primary-500" /> Theme</h3>
        <div className="flex gap-3">
          {[{ key: 'light', icon: Sun, label: 'Light' }, { key: 'dark', icon: Moon, label: 'Dark' }].map(({ key, icon: Icon, label }) => (
            <button key={key} onClick={() => dispatch(setTheme(key))} className={`flex items-center gap-2 px-5 py-3 rounded-xl border-2 ${theme === key ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-slate-200 dark:border-slate-700'}`}>
              <Icon className={`h-4 w-4 ${theme === key ? 'text-primary-500' : 'text-slate-400'}`} /> <span className="font-medium text-sm">{label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        <h3 className="font-semibold flex items-center gap-2"><Bell className="h-4 w-4 text-primary-500" /> Notifications</h3>
        {[{ key: 'email', label: 'Email' }, { key: 'push', label: 'Push' }, { key: 'sms', label: 'SMS' }].map(({ key, label }) => (
          <label key={key} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer">
            ...
            <input type="checkbox" checked={notifPrefs[key]} onChange={() => setNotifPrefs(p => ({ ...p, [key]: !p[key] }))} />
          </label>
        ))}
      </div>
    </motion.div>
