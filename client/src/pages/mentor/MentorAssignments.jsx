...
import { Plus, ClipboardCheck, Eye, Edit3, Trash2, Download } from 'lucide-react';
...
export const MentorAssignments = () => {
  ...
  const { data, isLoading } = useFetch(['mentor-assignments-list', page], () => assignmentService.getAll({ page, limit: 10 }));
  ...
  const openEdit = (a) => { setEditAssignment(a); setShowModal(true); };

  const columns = [
    { key: 'title', label: 'Title', render: (row) => <div><p className="font-medium text-sm">{row.title}</p><p className="text-xs text-slate-500">{row.course?.title}</p></div> },
    ...
    { key: 'submissions', label: 'Submitted', render: (row) => <span className="text-xs">{row.submissionCount || 0}</span> },
    { key: 'isPublished', label: 'Status', render: (row) => <Badge variant={row.isPublished ? 'success' : 'warning'} className="text-xs">{row.isPublished ? 'Open' : 'Closed'}</Badge> },
    { key: 'actions', label: '', render: (row) => (
      <div className="flex items-center gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); if (confirm('Delete?')) deleteMutation.mutate(row._id); }} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-400 hover:text-red-500"><Trash2 className="h-3.5 w-3.5" /></button>
      </div>
    )},
  ];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Assignments</h1><p className="text-slate-500">Create and grade assignments</p></div>
        <button onClick={() => { setEditAssignment(null); setShowModal(true); }} className="btn-primary flex items-center gap-2"><Plus className="h-4 w-4" /> New</button>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <StatsCard title="Total" value={assignments.length} icon={ClipboardCheck} color="primary" />
        <StatsCard title="Open" value={assignments.filter(a => a.isPublished).length} icon={Eye} color="secondary" />
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditAssignment(null); }} title={editAssignment ? 'Edit' : 'Create Assignment'}>
        <form onSubmit={(e) => {
          e.preventDefault();
          const fd = new FormData(e.target);
          const d = Object.fromEntries(fd);
          editAssignment ? updateMutation.mutate({ id: editAssignment._id, data: d }) : createMutation.mutate(d);
          setShowModal(false); setEditAssignment(null);
        }} className="space-y-4">
          <input name="title" placeholder="Title" className="input-field" required defaultValue={editAssignment?.title || ''} />
          <textarea name="description" placeholder="Description" className="input-field" rows={2} defaultValue={editAssignment?.description || ''} />
      ...
      <Modal isOpen={!!viewSubmissions} onClose={() => setViewSubmissions(null)} title={`Submissions: ${viewSubmissions?.title || ''}`} size="2xl">
        <div className="max-h-[60vh] overflow-y-auto space-y-3">
          {submissions.length === 0 ? <p className="text-center py-8 text-slate-500">No submissions</p> : submissions.map((sub) => (
            <div key={sub._id} className="border border-slate-200 dark:border-slate-700 rounded-xl p-4">
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div><p className="font-medium text-sm">{sub.student?.name || 'Unknown'}</p><p className="text-xs text-slate-500">{formatDate(sub.submittedAt)}</p></div>
                </div>
                {sub.status === 'graded' ? <Badge variant="success" className="text-xs">{sub.score}/{viewSubmissions?.totalPoints || 100}</Badge> : <Badge variant="warning" className="text-xs">Pending</Badge>}
              </div>
              ...
              {sub.status !== 'graded' && (
                <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-100 dark:border-slate-700">
                  <input type="number" placeholder="Score" className="input-field w-20 text-sm" value={gradeForm[sub._id]?.score || ''} onChange={(e) => setGradeForm(p => ({ ...p, [sub._id]: { ...p[sub._id], score: e.target.value } }))} />
                  <input placeholder="Feedback" className="input-field flex-1 text-sm" value={gradeForm[sub._id]?.feedback || ''} onChange={(e) => setGradeForm(p => ({ ...p, [sub._id]: { ...p[sub._id], feedback: e.target.value } }))} />
