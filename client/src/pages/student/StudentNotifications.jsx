...
export const StudentNotifications = () => {
  ...
  const { data } = useFetch('student-notifications-page', () => notificationService.getAll({ limit: 50 }));
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="glass rounded-2xl divide-y divide-slate-200 dark:divide-slate-700">
        {notifications.length === 0 ? <div className="p-8 text-center text-slate-500">All clear!</div> : notifications.map((n) => (
          <div key={n._id} className={cn('p-4 flex gap-3 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50', !n.isRead && 'bg-primary-50/50 dark:bg-primary-900/10')} onClick={() => { if (!n.isRead) { readMutation.mutate(n._id); dispatch(markAsRead(n._id)); } }}>
